...
const Sidebar = ({ isOpen, setIsOpen }) => {
    ...
    return (
        <aside
            className={`hidden md:flex flex-col bg-white border-l border-gray-200 shadow-sm fixed right-0 top-0 bottom-0 z-40 transition-all duration-300 
            ${isOpen ? "w-64" : "w-20"}`}
        >
                ...
                <div className="flex justify-between items-center px-4 py-4 border-b">
                    <Link to="/" className="flex justify-center w-full">
                        <img
                            src="/logo.svg"
                            alt="Fanan Logo"
                            className={`transition-all duration-300 ${isOpen ? "h-16" : "h-10"}`}
                        />
                    </Link>
                    {isOpen && (
                        <button
