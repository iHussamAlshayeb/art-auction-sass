...
const Sidebar = () => {
    ...
    // روابط القائمة المنسدلة للوحة التحكم
        ...
        <aside className="hidden md:flex flex-col w-64 bg-white border-l border-gray-200 shadow-sm fixed right-0 top-0 bottom-0 z-40">
            <div className="flex flex-col h-full">
                ...
                <nav className="flex flex-col flex-grow p-4 space-y-1 overflow-y-auto">
                    ...
                    {user && (
                        <div>
                            <button
                                ...
                                className={`w-full flex items-center justify-between px-4 py-2 rounded-lg text-sm font-medium transition-all ${location.pathname.startsWith("/dashboard") ? "bg-orange-100 text-orange-600" : "text-gray-700 hover:bg-orange-50 hover:text-orange-600"}`}
                            >
                                ...
                                {openMenus["dashboard"] && (
                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mr-4 mt-1 space-y-1 border-r-2 border-orange-100">
                                        {dashboardSubmenu.map((item) => (
                                            <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `block pl-8 pr-4 py-1.5 rounded-md text-sm transition-all ${isActive ? "bg-orange-50 text-orange-600 font-medium" : "text-gray-600 hover:text-orange-500"}`}>
                                                {item.label}
                                            </NavLink>
                                        ))}
                                        {user.role === 'STUDENT' && studentSubmenu.map((item) => (
                                            <NavLink key={item.to} to={item.to} className={({ isActive }) => `block pl-8 pr-4 py-1.5 rounded-md text-sm transition-all ${isActive ? "bg-orange-50 text-orange-600 font-medium" : "text-gray-600 hover:text-orange-500"}`}>{item.label}</NavLink>
                                        ))}
                                        {user.role === 'BUYER' && buyerSubmenu.map((item) => (
                                            <NavLink key={item.to} to={item.to} className={({ isActive }) => `block pl-8 pr-4 py-1.5 rounded-md text-sm transition-all ${isActive ? "bg-orange-50 text-orange-600 font-medium" : "text-gray-600 hover:text-orange-500"}`}>{item.label}</NavLink>
                                        ))}
                                    </motion.div>
                                )}
                    ...
                    {/* رابط لوحة الإدارة */}
                    {user?.role === "ADMIN" && (
                        <NavLink to="/admin" className={({ isActive }) => `flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-medium transition-all ${isActive ? "bg-red-100 text-red-600" : "text-red-500 hover:bg-red-50 hover:text-red-700"}`}>
                            <Shield size={18} />
