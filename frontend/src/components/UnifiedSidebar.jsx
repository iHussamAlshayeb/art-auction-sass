...
const UnifiedSidebar = () => {
    ...
    return (
        // هذه القائمة تظهر فقط على شاشات سطح المكتب
                ...
                <nav className="flex flex-col flex-grow p-4 space-y-1">
                    {mainLinks.map(({ to, label, icon }) => (
                        <NavLink
                            key={to}
                            to={to}
                            end={to === "/"} // 'end' prop for the homepage link
                            className={({ isActive }) => `flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-medium transition-all ${isActive ? "bg-orange-100 text-orange-600" : "text-gray-700 hover:bg-orange-50 hover:text-orange-600"}`}
                        >
                            {icon}
                            <span>{label}</span>
                        </NavLink>
                    ...
                    {/* روابط المستخدم المسجل دخوله */}
                    {user && (
                        <>
                            <hr className="my-4" />
                            <NavLink
                                to="/dashboard"
                                className={({ isActive }) => `flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-medium transition-all ${location.pathname.startsWith("/dashboard") || isActive ? "bg-orange-100 text-orange-600" : "text-gray-700 hover:bg-orange-50 hover:text-orange-600"}`}
                            >
                    ...
                    {user?.role === "ADMIN" && (
                        <NavLink
                            to="/admin"
                            className={({ isActive }) => `flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-medium transition-all ${isActive ? "bg-red-100 text-red-600" : "text-red-500 hover:bg-red-50 hover:text-red-700"}`}
                        >
                            <Shield size={18} />
